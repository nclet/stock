...
# ----------------------
st.set_page_config(page_title="Crypto Sentiment & Trend Predictor", layout="wide")
st.title("📊 코인 가격 예측 모형")

st.markdown("""
이 모델은 **기술적 지표(RSI, 볼린저밴드, MACD**, **뉴스 감성 전략**, **시장 심리(Fear & Greed Index)**,
**구글 트렌드** 등을 결합해 코인 가격을 예측합니다.
""")
...
def main():
    ...
    if st.button("🚀 하이브리드 모델 분석 시작", use_container_width=True):
        ...
        if X_test.empty:
            ...
            st.info("💡 **모델 해석:** `RSI`, `MACD`, `볼린저밴드`와 같은 기술적 지표와 더불어 뉴스 감성, Fear & Greed Index, 그리고 구글 트렌드 값이 가격 결정에 지대한 영향을 미칩니다.")

...
st.title("🌐 미국 거시경제 4분면 시장 국면 분석")
st.markdown("FRED 데이터를 활용하여 **성장(Growth)**과 **인플레이션(Inflation)** 기반의 4분면 국면을 분석하고, S&P 500 지수 추세를 예측합니다.")
...
def preprocess_and_engineer_features(df_fred, df_stocks):
    ...
    features = [
        'US_CPI_YoY',
        'US_CPI_YoY_MA_Change', # 🌟 인플레이션 추세 판단 기준
        'US_ISM',
        'US_ISM_MA_Change', # 🌟 성장 추세 판단 기준
        'US_FFR',
        'US_FFR_Change',
        'US_10Y_Treasury',
...
st.write("아래 버튼을 클릭하여 FRED (ISM 포함) 및 S&P 500 데이터를 수집하고 시장 국면 분석을 시작하세요.")

if st.button("🚀 **데이터 수집 및 4분면 분석 시작!**", key="start_analysis_button"):
    with st.spinner("데이터를 수집하고 분석 중입니다. 잠시만 기다려 주세요..."):
        ...
        with st.expander("🔍 4분면 시장 국면이란? (클릭하여 자세히 보기)", expanded=False):
            ...
            """)
            st.image('https://i.imgur.com/8Qn7p7R.png', caption='4분면 시장 국면 매트릭스 예시') # 매트릭스 이미지 추가
            st.write("---")
            st.markdown(
                """
        ...
        # 국면별 색상 정의
        regime_colors = {
            "Goldilocks (골디락스)": "green",
            "Overheat (과열)": "red",
            "Stagflation (스태그플레이션)": "orange",
            "Reflation (리플레이션)": "blue",
            "Unknown": "gray"
        }
        ...
        fig_scatter.update_layout(
            xaxis_title='성장 추세 (ISM MA Change)',
            yaxis_title='인플레이션 추세 (CPI MA Change)',
            legend_title='시장 국면'
        )
        ...
        fig_scatter.add_trace(go.Scatter(
            x=[latest_point['US_ISM_MA_Change']], 
            y=[latest_point['US_CPI_YoY_MA_Change']],
            mode='markers',
            marker=dict(size=15, color=regime_colors.get(latest_point['Market_Regime'], 'gray'), line=dict(width=3, color='black')),
            name=f"현재 국면: {latest_point['Market_Regime']}",
            hovertext=f"현재: {latest_point['Market_Regime']}<br>날짜: {latest_point.name.strftime('%Y-%m')}"
        ))
        ...
        st.markdown("---")
        st.subheader("📊 **3. 주요 거시경제 지표 추세 (최근 5년) (Plotly)**")
        
        plot_df = df_regime_classified.last('5Y')
        
        cols = st.columns(3)
        # ISM 지표를 포함하도록 업데이트
        us_features_to_plot = ['US_CPI_YoY', 'US_ISM', 'US_FFR', 'US_10Y_Treasury', 'US_CPI_YoY_MA_Change', 'US_ISM_MA_Change']

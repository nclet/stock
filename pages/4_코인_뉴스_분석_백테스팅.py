...
def get_fear_greed_index(limit=365):
    ...
    try:
        ...
        st.error(f"❌ Fear & Greed Index 오류: {e}")
        return pd.DataFrame()
...
def get_upbit_candles(market, start_date, end_date):
    ...
    return df_final[(df_final['Date'] >= start_date) & (df_final['Date'] <= end_date)][['Date', 'Close']]

# ------------------------
# ✨ 실행
# ------------------------
max_news = st.slider("최대 뉴스 건수", 10, 100, 30, step=10)

if st.button("🚀 크롤링 및 분석 시작"):
    # 뉴스 크롤링
    all_news = pd.DataFrame()
    for start_idx in range(1, max_news, 100):
    ...
    if filtered_news.empty:
        ...
        if len(X) > 5:
            ...
            df_merge['Predicted_Close'] = model.predict(X)

            st.subheader("📊 예측 결과 (감성 + 모멘텀)")
            fig, ax = plt.subplots(figsize=(12, 6))
            ax.plot(df_merge['Date'], df_merge['Close'], label='실제', color='blue')
            ax.plot(df_merge['Date'], df_merge['Predicted_Close'], '--', color='red', label='예측')

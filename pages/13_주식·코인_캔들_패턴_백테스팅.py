    ...
    if st.button("차트 분석 시작", type="primary", use_container_width=True):
        ...
        if df is not None and not df.empty:
            ...
            for pattern_label_with_initials in selected_patterns:
                ...
                # mplfinance의 범례에 들어갈 패턴 이니셜을 가져옵니다.
                pattern_initial = pattern_mapping[col_name]['initial']
                ...
                if col_name == 'is_three_white_soldiers':
                    ...
                    if not series.dropna().empty:
                        # 수정된 부분: label=pattern_initial
                        apds.append(mpf.make_addplot(series,
                                                     type='line', linestyle='solid', width=5, color='red', label=pattern_initial))
                    ...
                    if not series.dropna().empty:
                        # 수정된 부분: label=pattern_initial
                        apds.append(mpf.make_addplot(series,
                                                     type='line', linestyle='solid', width=5, color='blue', label=pattern_initial))
                else:
                    # 마커로 표시되는 패턴들

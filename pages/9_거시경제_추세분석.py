...
#     # 실업률 변화율 추가 (전년 동기 대비 변화율)
#     if 'KR_Unemployment_Rate' in df_merged.columns:
#         # 실업률은 보통 절대적인 변화폭을 보거나, 추세를 보는 경우가 많으므로 diff()를 사용하거나 그대로 사용할 수 있습니다.
#         # 여기서는 전년 동기 대비 변화를 백분율로 계산하겠습니다. (혹은 단순 차이)
#         df_merged['KR_Unemployment_Rate_YoY_Change'] = df_merged['KR_Unemployment_Rate'].diff(12) # 전년 동월 대비 변화 (절대값)
#     else: df_merged['KR_Unemployment_Rate_YoY_Change'] = np.nan
...
#         'US_CPI_YoY_Change', 
#         'KR_CPI_YoY_Change', 
#         'KR_Unemployment_Rate_YoY_Change', # 실업률 변화율 추가
#         'US_FFR', 'US_FFR_Change', 'KR_BaseRate', 'KR_BaseRate_Change',
#         'KRW_USD_ExcRate_Change',
#         'US_10Y_Treasury', 'US_10Y_Treasury_Change'
#     ]
...
#     # 성장 추세는 GDP를 사용하지 않으므로, 이 부분은 제거하거나 다른 지표로 대체할 수 있습니다.
#     # 여기서는 시장 국면 분류를 '물가 상승'과 '물가 하락' 두 가지로 단순화합니다.
#     # 실업률은 직접적인 시장 국면 분류보다는 추가적인 경제 건전성 지표로 활용합니다.
#     df_regime['Growth_Trend'] = True # 이 값은 현재 시장 국면 분류 로직에 영향을 미치지 않습니다.
...
#         ax_regime.set_title("시장 국면별 KOSPI 월별 평균 수익률 (%)")
#         ax_regime.set_xlabel("시장 국면")
#         ax_regime.set_ylabel("평균 월별 수익률 (%)")
...
#         # 전체 features 리스트를 사용하여 모든 지표를 시각화합니다.
#         cols = st.columns(3)
#         for i, feature in enumerate(features):
#             with cols[i % 3]:
#                 if feature in plot_df.columns:

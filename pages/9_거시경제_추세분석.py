...
# **TTL 값 연장 및 각 지표 호출 후 무작위 지연 추가**
...
def get_fred_data(api_key):
    ...
    max_fred_retries = 5 # FRED는 재시도 횟수를 조금 더 늘림
    ...
    for name, code in fred_codes.items():
        for attempt in range(max_fred_retries):
            try:
                ...
                # 'Access Denied'와 같은 특정 오류 메시지에 대한 경고를 강화
                error_msg = str(e)
                if "Access Denied" in error_msg or "403" in error_msg:
                    st.error(f"🔴 FRED API 접근 거부 오류 ({name}, {code}): {error_msg}. API 키 또는 요청 빈도를 확인하세요.")
                else:
                    st.warning(f"FRED 데이터 로드 오류 ({name}, {code}): {error_msg}.")
                
                if attempt < max_fred_retries - 1:
                    sleep_time = initial_fred_delay * (2 ** attempt) + random.uniform(0, 1)
                    st.info(f"{sleep_time:.2f}초 후 재시도... ({attempt + 1}/{max_fred_retries})")
                    time.sleep(sleep_time)
                else:
                    st.error(f"❌ FRED 데이터 로드 최종 실패: {name}, {code}. 오류: {error_msg}")
                    # 최종 실패 시 해당 컬럼은 NaN으로 남을 수 있음
        
        # **각 지표 호출 후 무작위 지연 추가**
        time.sleep(random.uniform(0.5, 1.5)) # FRED는 특히 더 긴 지연을 줌
...
def get_ecos_data(api_key):
    ...
    end_date_str_month = (now + timedelta(days=30)).strftime('%Y%m') # 월말 데이터 보정
    ...
    for name, info in ecos_codes_info.items():
        ...
        for attempt in range(max_ecos_retries):
            try:
                response = requests.get(url, timeout=15) # 타임아웃 15초로 늘림
                response.raise_for_status() # 200 OK가 아닌 경우 HTTPError 발생
                data = response.json()
                ...
                time.sleep(initial_ecos_delay * (2 ** attempt) + random.uniform(0, 1))
            except requests.exceptions.RequestException as e: # HTTPError 포함 상위 예외 처리
                st.warning(f"ECOS API 요청 오류 ({name}, {info['code']}): {e}")
                if 'response' in locals() and response.status_code == 403: # Access Denied 유사 오류
                    st.error(f"🔴 ECOS API 접근 거부 오류! API 키, 요청 빈도를 확인하거나 한국은행 ECOS 관리자에게 문의하세요.")
                if attempt < max_ecos_retries - 1:
                    sleep_time = initial_ecos_delay * (2 ** attempt) + random.uniform(0, 1)
                    st.info(f"{sleep_time:.2f}초 후 재시도... ({attempt + 1}/{max_ecos_retries})")
                    time.sleep(sleep_time)
                else:
                    st.error(f"❌ ECOS 데이터 로드 최종 실패: {name}. 요청 오류.")
            except Exception as e:
        ...
        # **각 지표 호출 후 무작위 지연 추가**
        time.sleep(random.uniform(0.5, 1.5)) # ECOS도 각 지표 호출 후 지연
...
# --- 데이터 전처리 및 파생 변수 생성 ---
@st.cache_data(ttl=3600 * 24 * 7) # 전처리 함수도 캐시 적용
def preprocess_and_engineer_features(df_fred, df_ecos, df_stocks):
    ...
    if not valid_dfs:
        ...
        return pd.DataFrame(), [], [] # 빈 리스트 반환하도록 수정
    ...
    if 'US_CPI_YoY' in df_merged.columns:
        # FRED의 CPIAUCSL은 지수값. 전년 동기 대비 변화율 계산 필요 (월간 데이터이므로 12개월 전과 비교)
    ...
    if 'KR_CPI_YoY' in df_merged.columns:
        # ECOS의 901Y009/0101000은 지수값. 전년 동기 대비 변화율 계산 필요
    ...
    # 모든 데이터를 병합한 후, 필요한 피처와 타겟을 선택하고 결측치 제거
    # 이 부분에서 df_merged가 비어있지 않다는 가정하에 진행
    if not df_merged.empty and actual_features and actual_targets:
        df_final = df_merged[actual_features + actual_targets].dropna()
    else:
        df_final = pd.DataFrame() # 데이터가 없으면 빈 데이터프레임 반환

    st.success("✅ 데이터 전처리 및 팩터 생성 완료!")
    return df_final, actual_features, actual_targets

# --- 시장 국면 정의 및 예측 모델 ---
@st.cache_data(ttl=3600 * 24 * 7) # 시장 국면 정의 함수도 캐시 적용
def define_market_regime(df):
    ...
    def classify_regime(row):
        ...
        if row['Inflation_Trend']:
            return "Inflationary Period (물가상승기)"
        else:
            return "Disinflationary Period (물가하락기)"
...
if st.button("🚀 **데이터 수집 및 분석 시작!**", key="start_analysis_button"):
    with st.spinner("데이터를 수집하고 분석 중입니다. 잠시만 기다려 주세요..."):
        ...
        if df_fred.empty and df_ecos.empty and df_stocks.empty: # 모든 데이터가 비어있을 때만 에러
            st.error("⚠️ 필수 데이터(FRED, ECOS, 주식 지수) 중 모든 데이터를 성공적으로 로드하지 못했습니다. 위의 경고 메시지(API 키, 인터넷 연결 등)를 확인하세요.")
            st.stop()
        ...
        # KOSPI 국면별 수익률
        with col1:
            if 'KOSPI_Next_Month_Return' in df_regime_classified.columns and not df_regime_classified['KOSPI_Next_Month_Return'].isnull().all():
                ...
                ax_kospi_regime.set_ylabel("월별 평균 수익률 (%)")
                ...
                st.warning("KOSPI 시장 국면별 수익률 데이터를 분석할 수 없습니다. 데이터가 부족하거나 'KOSPI_Next_Month_Return' 컬럼에 유효한 값이 없습니다.")

        # 미국 주식 시장 (S&P 500) 국면별 수익률
        with col2:
            if 'US_Next_Month_Return' in df_regime_classified.columns and not df_regime_classified['US_Next_Month_Return'].isnull().all():
                ...
                ax_us_regime.set_ylabel("월별 평균 수익률 (%)")
                ...
                st.warning("S&P 500 시장 국면별 수익률 데이터를 분석할 수 없습니다. 데이터가 부족하거나 'US_Next_Month_Return' 컬럼에 유효한 값이 없습니다.")
        ...
        for i, feature in enumerate(features):
            with cols[i % 3]:
                if feature in plot_df.columns and not plot_df[feature].isnull().all():
                    ...
                    st.write(f"⚠️ **'{feature}'** 지표 데이터가 존재하지 않거나 모두 결측치여서 시각화할 수 없습니다.")
        ...
        # 상관관계 분석 전에 실제로 데이터가 있는지 확인
        if features and targets and not df_final[features + targets].empty:
            ...
            ax_corr.set_title("거시경제 지표 변화와 주식 시장 수익률 간의 상관관계")
            plt.xticks(rotation=45, ha='right')
            plt.yticks(rotation=0)
            plt.tight_layout()
            st.pyplot(fig_corr)
        else:
            st.warning("상관관계 히트맵을 생성할 충분한 데이터가 없습니다. 거시경제 지표와 주식 수익률 데이터를 확인하세요.")
        ...
        if 'KOSPI_Next_Month_Return' in df_regime_classified.columns and latest_regime in kospi_regime_performance.index:
            ...
            st.warning("KOSPI 시장의 현재 국면 예상 수익률을 계산할 수 없습니다. 관련 데이터가 충분한지 확인하세요.")
        ...
        if 'US_Next_Month_Return' in df_regime_classified.columns and latest_regime in us_regime_performance.index:
            ...
            st.warning("S&P 500 시장의 현재 국면 예상 수익률을 계산할 수 없습니다. 관련 데이터가 충분한지 확인하세요.")

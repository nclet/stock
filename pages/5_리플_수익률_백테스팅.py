...
st.title("📈 리플(XRP) 백테스팅 (장·단기 이평선, 모멘텀, RSI)") # 비트코인 -> 리플 (XRP)
st.write("##### RSI와 모멘텀 지표 설정은 좌측 사이드바 메뉴에서 가능합니다.")

# 기본 날짜 설정
default_end_date = datetime.date.today()
# 업비트 XRP/KRW는 2017년 10월 16일부터 데이터가 있습니다. (정확한 날짜 확인 필요)
# 비트코인보다 늦게 상장된 경우가 많으므로 날짜를 조정합니다.
min_valid_date_for_upbit_xrp = datetime.date(2017, 10, 16) # XRP 상장일로 변경 (대략적인 날짜, 실제 확인 필요)
...
if start_date >= end_date:
    ...
    st.stop()
elif start_date < min_valid_date_for_upbit_xrp: # 최소 유효 날짜 경고 메시지 변경
...
# XRP/KRW 사용 (업비트)
st.write(f"##### 데이터 다운로드: **XRP/KRW** ({start_date} ~ {end_date})") # BTC -> XRP
ohlcv_data = load_crypto_data("XRP/KRW", "1d", start_date, end_date) # BTC -> XRP

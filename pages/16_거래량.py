...
# 페이지 설정
# --------------------------
st.set_page_config(page_title="시장별 거래량/거래대금 분석", layout="wide")

st.title("📊 시장별 거래량/거래대금 분석 (pykrx)")
...
# --------------------------
@st.cache_data
def load_data(start, end, market):
    try:
        ...
        return df_value, df_volume
    except Exception as e:
        st.error(f"데이터 로드 중 오류 발생: {e}")
        return None, None

df_value, df_volume = load_data(start, end, markets[selected_market])

# --------------------------
# 시각화
# --------------------------
if df_value is None or df_volume is None or df_value.empty or df_volume.empty:
    st.warning("선택한 시장과 기간에 대한 데이터를 찾을 수 없습니다.")
else:
    tab1, tab2 = st.tabs(["💵 거래대금", "📈 거래량"])

    with tab1:
        st.subheader(f"{selected_market} 거래대금 (단위: 억 원)")
        fig1 = px.line(
            df_value,
            x="날짜",
            y=df_value.columns[1:],  # 기관/외국인/개인 등
            title=f"{selected_market} 거래대금 추이",
        )
        st.plotly_chart(fig1, use_container_width=True)

    with tab2:
        st.subheader(f"{selected_market} 거래량 (단위: 주식 수)")
        fig2 = px.line(
            df_volume,
            x="날짜",
            y=df_volume.columns[1:],  # 기관/외국인/개인 등
            title=f"{selected_market} 거래량 추이",
        )

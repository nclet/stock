...
# 🚨 순매수/순매도 금액을 제공하는 정확한 함수: get_market_investor_trade_trend 사용
from pykrx.stock import get_market_investor_trade_trend 

# --------------------------
# 페이지 설정
# --------------------------
st.set_page_config(layout="wide", page_title="pykrx 기반 코스피/코스닥 순매수 자금 흐름 분석")

# --------------------------
# 앱 제목 및 설명
# --------------------------
st.title("💰 KOSPI/KOSDAQ 매매 주체별 순매수 자금 흐름 분석 (pykrx)")
st.markdown("""
이 대시보드는 **pykrx** 라이브러리를 사용하여 코스피(KOSPI)와 코스닥(KOSDAQ) 시장의  
**개인, 기관, 외국인**의 일별 **순매수/순매도 금액** (원 단위) 추이를 시각화합니다.  
...
        start_str, end_str = start_date.strftime("%Y%m%d"), end_date.strftime("%Y%m%d")
        ...
        # 순매수 금액 데이터 로드
        df_raw = get_market_investor_trade_trend(start_str, end_str, market)

        if df_raw.empty:
            return pd.DataFrame()

        # 순매수 데이터 컬럼: '개인', '외국인합계', '기관합계' 사용
        df_market = df_raw[["개인", "외국인합계", "기관합계"]].copy()
        # 컬럼명 간소화
        df_market.rename(
            columns={
                "외국인합계": "외국인", 
...
        df_long.rename(columns={"날짜": "Date"}, inplace=True)
        ...
        # pykrx의 순매수 금액은 원 단위이므로, 시각화를 위해 억 원으로 단위를 변환합니다.
        # 1억 = 100,000,000
        df_long['Net_Flow_억'] = df_long['Net_Flow'] / 100000000.0
...
    with col3:
        # pykrx는 장 마감일까지만 데이터 제공하므로 당일 제외
...
    else:
        with st.spinner(f"{selected_market_name} 매매 주체별 순매수 자금 흐름 데이터를 불러오는 중..."):
...
        if df_long.empty:
            st.warning("선택한 시장과 기간에 대한 데이터를 찾을 수 없습니다. (날짜 범위를 확인해 주세요.)")
        else:
...
                df_long,
                x="Date",
                y="Net_Flow_억", # 억 원 단위로 시각화
                color="Investor",
                title=f"{selected_market_name} 일별 매매 주체별 순매수/순매도 (단위: 억 원)",
                labels={"Net_Flow_억": "순매수 금액 (억 원)", "Date": "날짜", "Investor": "투자 주체"},
                template="plotly_white",
                barmode="relative"
            )

            # 순매수/순매도는 0을 기준으로 위/아래로 표시되어야 하므로 0 기준선 추가
            fig.update_layout(
                xaxis_title="날짜",
                yaxis_title="순매수 금액 (억 원)",
                hovermode="x unified",
                shapes=[
                    dict(
                        type="line",
                        xref="paper", yref="y",
                        x0=0, y0=0, x1=1, y1=0,
                        line=dict(color="red", width=1.5) # 0 기준선을 더 강조
                    )
                ]
            )
...
            # Wide Format 테이블
            st.subheader("📋 데이터 미리보기 (순매수 금액, 억 원 단위)")
            df_wide_억 = df_long.pivot(index="Date", columns="Investor", values="Net_Flow_억")
            st.dataframe(df_wide_억.sort_index(ascending=False).style.format('{:,.2f}'), use_container_width=True)

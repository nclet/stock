...
with st.container():
    ...
    with col3:
        end_date = st.date_input("🗓️ 종료 날짜", today)
    ...
    if start_date > end_date:
        ...
        with st.spinner(f"{selected_market_name} 매매 주체별 자금 흐름 데이터를 불러오는 중..."):
            df_long = load_investor_data(
                PARKX_MAPPING[selected_market_name],
                start_date,
                end_date
            )
        ...
        if df_long.empty:
            ...
            fig = px.bar(
                ...
                y="Net_Flow",
                color="Investor",
                title=f"{selected_market_name} 일별 매매 주체별 순매수/순매도 (단위: 억원)",
                labels={"Net_Flow": "순매수 금액 (억원)", "Date": "날짜", "Investor": "투자 주체"},
